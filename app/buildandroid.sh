...
GOOS=android  GOARCH=arm  CGO_LDFLAGS="-arch arm -lmy_project" CGO_ENABLED=1 go build -v -x -buildmode=c-shared -o ../fedimui.go.so
# GOOS=android GOARCH=arm64 CGO_ENABLED=1 go build -v -x -buildmode=c-shared
# otool -L ../fedimui.rs.so | grep my_project
$NDKDIR/bin/llvm-objdump -x ../fedimui.go.so |grep NEEDED
